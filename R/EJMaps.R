...
EJMaps <- function(input_data, indic_option = NULL, perc_geog = NULL,
                   input.name = NULL, save_options = F){
  ...
  for (i in 1:length(input_data$EJ.facil.data)){
    ...
    if (ind.option == 'total'){
      ...
      EJ.maps[[names(input_data$EJ.facil.data)[i]]] <-
        ...
        leaflet::addCircleMarkers(radius = 5,
                         ...
                         opacity = 0.75,
                         popup = leafpop::popupTable(dplyr::filter(na.omit(map.data), geography == geog.ind),
    ...
    if (ind.option == 'environmental'){
      ...
      # Add facility name if given by user.
      if(!is.null(input.name) &
         (length(input.name) == dim(input_data$EJ.facil.data[[i]])[1]/2)){
        name.id <- as.data.frame(input.name) %>%
          tibble::rowid_to_column('shape_ID')
        names(name.id)[2] <- 'LocationID'
        map.data <- map.data %>%
      ...
      pal <- leaflet::colorFactor(
        rev(RColorBrewer::brewer.pal(n=11, "Spectral")),
        domain = na.omit(input_data$EJ.facil.data[[i]])$`Env. indicators above 80th %ile`)

      # Leaflet object
      EJ.maps[[names(input_data$EJ.facil.data)[i]]] <-
        leaflet::leaflet(data = sf::st_as_sf(dplyr::filter(na.omit(input_data$EJ.facil.data[[i]]), geography == 'US'),
                                crs = 4326)) %>% 
        leaflet::addTiles() %>%
        leaflet::fitBounds(lng1 = min(sf::st_coordinates(sf::st_as_sf(input_data$EJ.facil.data[[i]]))[,1]),
                  lat1 = min(sf::st_coordinates(sf::st_as_sf(input_data$EJ.facil.data[[i]]))[,2]),
                  lng2 = max(sf::st_coordinates(sf::st_as_sf(input_data$EJ.facil.data[[i]]))[,1]),
                  lat2 = max(sf::st_coordinates(sf::st_as_sf(input_data$EJ.facil.data[[i]]))[,2])) %>%
        leaflet::addCircleMarkers(radius = 5,
                         color = ~ pal(`Env. indicators above 80th %ile`),
                         opacity = 0.75,
                         #clusterOptions = markerClusterOptions(),
                         popup = leafpop::popupTable(dplyr::filter(na.omit(input_data$EJ.facil.data[[i]]), geography == geog.ind),
                                            feature.id = F, row.numbers = F,
                                            zcol = names(na.omit(input_data$EJ.facil.data[[i]]))[1:20])) %>%
        leaflet::addLegend(pal = pal, values = ~`Env. indicators above 80th %ile`, position = "bottomright")
    }

    if (ind.option == 'demographic'){

      map.data <- input_data$EJ.facil.data
      ...
      pal <- leaflet::colorFactor(
        rev(RColorBrewer::brewer.pal(n=11, "Spectral")),
        domain = na.omit(input_data$EJ.facil.data[[i]])$`Demo. indicators above 80th %ile`)
