...
EJCountTable <- function(input.data, save.option = F){
  ...
  for (i in 1:length(input.data$EJ.facil.data)){
    EJ.count.tables[[names(input.data$EJ.facil.data)[i]]] <-
      proc_freq(filter(input.data$EJ.facil.data[[i]], geography == 'US'), 'Env. indicators above 80th %ile', 'Demo. indicators above 80th %ile',
    ...
    if (save.option == T){
      ...
      save_as_image(x = EJ.count.tables[[names(input.data$EJ.facil.data)[i]]],
