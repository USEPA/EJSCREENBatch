...
First, the user supplies input location data. Since this is a batch tool, the data can (and should!) include several input locations. These locations could be points in space (i.e. lat/long coordinates of an emitting facility), boundary shapes (i.e. a set of polygons represeting wetlands or lagoons), or a waterbody identifier from the NHDPlus database (more on this below).

Here, as a toy data set, we draw 20 random latitude/longitude coordinates from the continental USA. These points will serve as our "locations of interest" for the vignette's sample EJ analysis.
...
For a illustrative sense of the geographies that will be screened, we map our (toy data's) location inputs below. In the next section, given these starting locations, we draw spatial buffers that are used to extract demographic and environmental data from EJSCREEN's national database.
