...
Second, the location-level percentile summaries are returned as *EJ.facil.data*. The dataframe includes both national and state percentiles for each EJSCREEN indicator. Under the default GIS option ("robust"), these are the percentiles for a population-weighted average of a location's indicator values.^[For other GIS options, a location's returned values are the median percentile across all of its CBGs.] Here, we call a return of the summary for the first two locations in our list. Four rows are returned, one for U.S. and one for state percentiles, per location:
...
ggdraw() + 
  draw_image("plots/Indexes_boxplot_gis_robust_radius1_state.jpeg", width = 0.5) +   
  draw_image("plots/demographics_boxplot_gis_robust_radius1_state.jpeg", width = 0.5, x = 0.5)
```
...
```{r, echo = F, warning =F, message = F, eval = F}
ggdraw() + 
  draw_image("plots/correlations_ej_indexes_gis_robust_radius1.jpeg", width = 0.6) +   
  draw_image("plots/correlations_demo_indexes_gis_robust_radius1.jpeg", width = 0.4, x = 0.5)
```
