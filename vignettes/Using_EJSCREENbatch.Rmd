...
The EJSCREENbatch R package primarily relies on data provided by [EJSCREEN](https://www.epa.gov/ejscreen), a mapping and screening tool maintained by EPA that provides demographic and environmental impact data on the Census Block Group (CBG) level for the United States. For information on how these data were prepared, refer to the [EJSCREEN Technical Information Guidance](https://www.epa.gov/ejscreen/technical-documentation-ejscreen). EJSCREENbatch also draws CBG-level demographic data from the Census Bureau's American Community Survey (ACS) using the [tidycensus](https://walker-data.com/tidycensus/index.html) R package.
...
Lastly: as of v2.0 you must point the package's EJfunction() towards the filepath where this population raster is stored locally. We suggest denoting an object (raster.path) with the filepath housing the SEDAC raster.^[Other note: higher resolution (e.g. 30m by 30m) US population density rasters have been produced by various research groups, including USEPA's Office of Research and Development, USGS, and Facebook. These granular rasters could be downloaded in lieu of the SEDAC raster, but standard accuracy versus computation time and feasibility trade-offs apply. Our functions simply look for and use a raster file named 'uspop10.tif' in the user-indicated filepath, so users should name alternative rasters using this convention.]
...
# Set directory and local pointer to SEDAC raster file.
  ...
  if(user=="aelkhatt"){
    ...
    setwd("C:/Users/atheisin/Environmental Protection Agency (EPA)/ORISE Check-In Group - General/EJScreen/Analyses for outside users/lagoon_EJScreen")
...
For illustrative purposes, we will work through this vignette using a dataset containing the latitude/longitude coordinates for 284 meat and poultry processing plants in the continental US. These points will serve as our "locations of interest" (LOIs) for the vignette's sample EJ analysis.
...
This land-based run *(data_type = 'landbased')* draws a simple buffer of 1 mile *(buff_dist = 1)* around each location, and extracts/returns the raw EJSCREEN data for all CBGs that intersect with the buffer area. Under the default (and suggested) GIS method *(gis_option = 'intersection')*, the function also returns a location-level summary data frame that is essentially equivalent to objects returned from the [EJSCREEN mapper](https://ejscreen.epa.gov/mapper/) or API.^[Our population weighting approach currently differs slightly from the EJSCREEN mapper's because we rely on the SEDAC population grid, while they weight populations using Census block geographies. Nevertheless, benchmarking exercises suggest very similar results in practice.]

This function returns 3 outputs: 
 1) *my.EJ.data*: a list of sub-objects (see below for details)
 
 2) *acs.cbg.data*: a dataframe of select ACS demographic data for the continential U.S.
 
 3) *data.state.uspr*: a data frame of the raw EJSCREEN for the continential U.S. 
...
my.EJ.data <- EJfunction(data_type = "landbased",
           ...
           raster_data = raster.path)
```
2.	The buffer GIS option used to extract CBG-level data from EJSCREEN. Users specify this via the *gis_option* argument. The default setting, described above, is “intersection”. The second option is "intersect": this extracts data from all CBGs that spatially intersect with the buffer region, but does not take any population-weighting measures to address areas that fall outside the buffer.

3.  The threshold argument used to identify elevated indicators and populations expressing characteristics of concern. This is set with the *threshold* argument. The default setting is the 80th percentile, as [initially suggested](https://www.epa.gov/ejscreen/frequent-questions-about-ejscreen) by EJSCREEN. 
...
```{r, cache = T, results = 'hide', warning = F, eval = F}
