...
download.file('https://adamtheising.github.io/Papers/uspop10.tif', '~/US Census Grid_SF2010_TIFF/uspop10.tif')
raster.path <- '~/DIRECTORY_NAME_HERE/US Census Grid_SF2010_TIFF'
```

Lastly: as of v2.0 you must point the package's EJfunction() towards the filepath where this population raster is stored locally. We suggest denoting an object (raster.path) with the filepath housing the SEDAC raster.^[Other note: higher resolution (e.g. 30m by 30m) US population density rasters have been produced by varied research groups, including USEPA's Office of Research and Development, USGS and Facebook. These granular rasters could be downloaded in lieu of the SEDAC raster, but standard accuracy versus computation time/feasibiliy tradeoffs apply. Our functions simply look for and use a raster file named 'uspop10.tif' in the user-indicated filepath, so users should name alternative rasters using this convention.]
...
For illustrative purposes, we will work through this vignette using a dataset containing the latitude/longitude coordinates for 284 meat and poultry processing plants in the continental US. These points will serve as our "locations of interest" for the vignette's sample EJ analysis.
...
catch.id <- vector(mode = "list", length = dim(mpp)[1])
for (i in 1:dim(mpp)[1]){
  catch.id[[i]] <- tryCatch(
    nhdplusTools::get_nhdplus(AOI = mpp[i,],
